...
type SeverityKey = "critical" | "high" | "medium" | "passed";
...
// Finding copy
const findingsCatalog: Record<FindingKey, FindingInfo> = {
  auth: {
    id: "AUTH-001",
    severity: "critical",
...
const scoreRingProgress = document.getElementById("scoreRingProgress") as SVGCircleElement | null;
...
const sections = document.querySelectorAll<HTMLDivElement>(".dashboard-section");
...
const lineChart = document.getElementById("lineChart") as SVGElement | null;
const donutChart = document.getElementById("donutChart") as SVGElement | null;
...
function renderScoreRing() {
  if (!scoreRingProgress || !scoreValue) return;
...
function renderDonutChart() {
  if (!donutChart || !donutLegend) return;
...
function renderLineChart() {
  ...
  lineChart.innerHTML = "";
  if (defs) lineChart.appendChild(defs);
...
function renderCodeExplorer(findingKey: FindingKey) {
  if (!codeEditorGrid || !activeFileName) return;
...
function simulateScanProcess() {
  if (!runScanButton || !scanOutput) return;
  ...
  setTimeout(() => {
    ...
    if (currentReport.history[currentReport.history.length - 1] !== 88) {
      ...
      if (currentReport.history.length > 10) currentReport.history.shift();
    }

    // Refresh UI metrics
    if (criticalCount) criticalCount.textContent = "0";
    if (highCount) highCount.textContent = "2";
    if (mediumCount) mediumCount.textContent = "4";
    if (passedCount) passedCount.textContent = "19";
    ...
    const highFinding = findingsList?.querySelector('[data-finding="events"]') as HTMLButtonElement | null;
    if (highFinding) {
      highFinding.click();
    }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (scanRes.ok) {
        ...
        if (criticalCount) criticalCount.textContent = currentReport.critical.toString();
        if (highCount) highCount.textContent = currentReport.high.toString();
        if (mediumCount) mediumCount.textContent = currentReport.medium.toString();
        if (passedCount) passedCount.textContent = currentReport.passed.toString();
    ...
    if (statusLabel) statusLabel.textContent = "Offline (Mock)";
  }
}

// 6. Search Bar Filter Logic
function filterFindings(query: string) {
  const findings = document.querySelectorAll<HTMLButtonElement>(".finding");
  ...
  for (const finding of findings) {
    ...
    if (matches) {
      ...
      if (!firstVisible) firstVisible = finding;
    } else {

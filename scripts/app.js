...
const findingCopy = {
    auth: {
        ...
        body: "This function writes contract state but does not require caller authorization. Add `caller.require_auth()` before accepting sensitive input.",
    ...
    events: {
        ...
        body: "This function updates contract state silently. Emitting a Soroban event makes the change traceable for indexers, dashboards, and auditors.",
    ...
    storage: {
        ...
        body: "The scanner found a collection written to instance storage without a visible cap or pruning path. Consider bounded keys, temporary storage, or archival strategy.",
...
findings.forEach((finding) => {
    finding.addEventListener("click", () => {
        ...
        if (detailCode)
            detailCode.textContent = copy.code;
    });
});
